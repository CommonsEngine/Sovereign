...
export async function getUserPluginSnapshots(userIds = [], { prisma, includeDisabled } = {}) {
  ...
  const overrides = await prisma.userPlugin.findMany({
    where: { userId: { in: ids } },
    select: {
      userId: true,
      pluginId: true,
...
export async function applyUserPluginUpdates(
  ...
  if (!normalized.length) {
    return { updated: 0, snapshot: await getUserPluginSnapshot(userId, { prisma }) };
  }
  ...
  const existingOverrides = await prisma.userPlugin.findMany({
    where: { userId },
  ...
  for (const entry of normalized) {
    ...
    if (!plugin) {
      logger?.warn?.(`[user-plugins] Unknown plugin in update payload`, entry);
      continue;
    }
