...
_Version 0.11 — June 2026. Changes from v0.10: Recorded the dev email-capture strategy — Mailpit for both Docker (compose service) and non-Docker (native binary) development, with Ethereal as the no-install fallback; the mailer stays config-only (no mock-mode). Decision log updated._
