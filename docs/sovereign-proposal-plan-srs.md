...
_Version 0.12 — June 2026. Changes from v0.11: Recorded the design-system decisions — monochrome identity, dark mode via `[data-theme]` (themes swap semantic tokens only), tsup external-CSS/React build for `packages/ui`, React pinned via the pnpm catalog, and the Vitest + Testing Library + jsdom component-testing setup. Decision log updated._
