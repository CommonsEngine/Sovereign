...
_Version 0.13 — June 2026. Changes from v0.12: Recorded the SDK interface-stub model — v1 surfaces throw `NotImplementedError` until the runtime injects real implementations, post-v1 surfaces are reserved, and `DrizzleClient` is opaque at the contract level (no dialect dependency in the published SDK). Decision log updated._
