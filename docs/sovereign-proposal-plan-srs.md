...
All three sovereign plugins target SDK compatibility with Sovereign v0.4+ (once Console ships and the SDK `db` implementation is stable).
...
_Version 0.16 — June 2026. Changes from v0.15: Recorded the runtime session-verification flow (§3.4/§3.10) — middleware → `/api/verify` → injected user headers, with `SOVEREIGN_AUTH_SECRET` deferred to v0.5 — and the generated-registry committed-placeholder convention. Decision log updated._
