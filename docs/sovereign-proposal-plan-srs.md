...
**Root plugin:** The platform has a configurable `root_plugin_id` (default: `fs.sovereign.launcher`). Navigating to `/` **serves the root plugin in place** — the runtime rewrites `/` to the root plugin's `routePrefix`, so the URL stays `/` while the plugin renders, and the plugin remains reachable at its own prefix. An admin can change this to any installed, enabled, non-adminOnly plugin via Console (CON-11). The first middle-section sidebar icon always resolves the current root plugin's icon and routes to `/`.
